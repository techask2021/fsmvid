 ...
 *
 * Usage POST: /api/weibo-proxy with body: { url: "https://f.video.weibocdn.com/..." }
 * Usage GET: /api/weibo-proxy?url=https://f.video.weibocdn.com/...
 *
 * Why needed:
 ...
 *
 * Note: POST method is preferred to avoid Cloudflare WAF blocking long URLs in query params
 */

// Helper function to process the proxy request
async function proxyWeiboVideo(request: NextRequest, videoUrl: string) {
  // Apply rate limiting to prevent abuse
  const rateLimitResult = await withRateLimit(request, RATE_LIMITS.DOWNLOAD)
  if (!rateLimitResult.success) {
    console.info(`[RATE LIMIT] Weibo proxy blocked`)
...
// GET method - for direct browser access (may be blocked by Cloudflare WAF for long URLs)
export async function GET(request: NextRequest) {
  try {
    ...
    return await proxyWeiboVideo(request, videoUrl)
  } catch (error) {
    console.error('[WEIBO PROXY GET] Error:', error)
    return new Response('Proxy error', { status: 500 })
  }
}

// POST method - preferred method to avoid Cloudflare WAF blocking
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[WEIBO PROXY POST] Error:', error)
    return new Response('Proxy error', { status: 500 })
  }
}
