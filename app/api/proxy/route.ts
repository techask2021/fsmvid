...
export async function POST(request: NextRequest) {
  try {
    ...
    // Normalize Bilibili URLs
    if (platform === 'bilibili' || (platform === 'universal' && (url.includes('bilibili.com') || url.includes('b23.tv')))) {
      if (url.includes('b23.tv/')) {
        // b23.tv is a short URL, let it pass as is or let the API handle it
      } else if (url.includes('bilibili.com/video/')) {
        // Extract the BV id and remove parameters
        const match = url.match(/bilibili\.com\/video\/(BV[a-zA-Z0-9]+)/);
        if (match) {
          processUrl = `https://www.bilibili.com/video/${match[1]}`;
          if (DEBUG_MODE) console.log("✅ Normalized Bilibili URL:", processUrl);
        }
      }
    }
